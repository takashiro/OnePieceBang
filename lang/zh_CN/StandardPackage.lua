...
出牌时机：出牌阶段。\
使用目标：除你以外的所有角色。\
作用效果：按行动顺序结算，除非目标角色打出1张【闪】，否则该角色受到【霸气】对其造成的1点伤害",
	["haou-haki-jink"] = "%src 使用了【霸王色霸气】，请打出一张【闪】以响应",
...
出牌时机：出牌阶段。\
使用目标：你。\
作用效果：将【雷天候】放置于你判定区里，目标角色回合判定阶段，进行判定；若判定结果为黑桃2-9之间（包括黑桃2和9），则【雷天候】对目标角色造成3点伤害，将闪电置入弃牌堆。若判定结果不为黑桃2-9之间（包括黑桃2和9），将【雷天候】移动到当前目标角色下家（【雷天候】的目标变为该角色）的判定区",

	["rain"] = "雨天候",
	[":rain"] = "延时锦囊牌\
出牌时机：出牌阶段。\
使用目标：你。\
作用效果：将【雨天候】放置于你判定区里，目标角色回合判定阶段，进行判定；若判定结果为红桃2-9之间（包括红桃2和9），则【闪电】对目标角色造成3点伤害，将闪电置入弃牌堆。若判定结果不为黑桃2-9之间（包括黑桃2和9），将【雨天候】移动到当前目标角色下家（【雨天候】的目标变为该角色）的判定区",

	["tornado"] = "风天候",
	[":tornado"] = "延时锦囊牌\
出牌时机：出牌阶段。\
使用目标：你。\
作用效果：将【风天候】放置于你判定区里，目标角色回合判定阶段，进行判定；若判定结果为梅花2-9之间（包括梅花2和9），目标角色弃置其装备区内所有牌，将闪电置入弃牌堆。若判定结果不为黑桃2-9之间（包括黑桃2和9），将【风天候】移动到当前目标角色下家（【风天候】的目标变为该角色）的判定区",
}
