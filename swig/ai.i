...
public:
	...
	virtual const Card *askForSingleWine(ServerPlayer *dying) = 0;
};
...
public:
	...
	virtual const Card *askForSingleWine(ServerPlayer *dying) ;
...
const Card *LuaAI::askForSingleWine(ServerPlayer *dying){
	lua_State *L = room->getLuaState();
	...
	if(error){
		...
		return TrustAI::askForSingleWine(dying);
	}
	const char *result = lua_tostring(L, -1);
	lua_pop(L, 1);
	if(result == NULL)
