...
sgs.ai_skill_use_func.ZhihengCard = function(card, use, self)
	...
	for index = #unpreferedCards, 1, -1 do
		if self.player:isJilei(sgs.QPirate:getCard(unpreferedCards[index])) then table.remove(unpreferedCards, index) end
	end
...
sgs.ai_skill_use_func.LijianCard=function(card,use,self)
	...
	if not self.player:hasUsed("LijianCard") then
		...
		local duel = sgs.QPirate:cloneCard("duel", sgs.Card_NoSuit, 0)
		for _, enemy in ipairs(self.enemies) do
