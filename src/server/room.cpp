...
void Room::outputEventStack(){
	...
	foreach(EventTriplet triplet, *driver->getEventStack()){
...
void Room::enterDying(ServerPlayer *player, DamageStruct *reason){
	...
	broadcastNotification(BP::PlayAudio, sos_filename);
	...
	driver->trigger(Dying, player, dying_data);
}
...
void Room::revivePlayer(ServerPlayer *player){
	...
	broadcastNotification(BP::RevivePlayer, player->objectName());
	updateStateItem();
}
...
void Room::killPlayer(ServerPlayer *victim, DamageStruct *reason){
	...
	broadcastNotification(BP::KillPlayer, victim->objectName());

	if(driver->trigger(GameOverJudge, victim, data)) return;

	driver->trigger(Death, victim, data);
...
void Room::judge(JudgeStruct &judge_struct){
	...
	driver->trigger(StartJudge, judge_star->who, data);
	...
	foreach(ServerPlayer *player, players){
		if(driver->trigger(AskForRetrial, player, data))
			break; //if a skill break retrial.
	}

	driver->trigger(FinishJudge, judge_star->who, data);
	driver->delay(delay);
}
...
void Room::gameOver(const QString &winner){
	...
	if(Config.ContestMode){
		foreach(ServerPlayer *player, m_players){
			...
			broadcastNotification(BP::SetScreenName, arg);
		}
	...
	broadcastNotification(BP::GameOver, arg);
	throw GameFinished;
}
...
void Room::slashEffect(const SlashEffectStruct &effect){
	...
	bool broken = driver->trigger(SlashEffect, effect.from, data);
	if(!broken)
		driver->trigger(SlashEffected, effect.to, data);
}
...
void Room::slashResult(const SlashEffectStruct &effect, const Card *jink){
	...
	if(jink == NULL){
		driver->trigger(SlashHit, effect.from, data);
	}else{
		setEmotion(effect.to, "jink");
		driver->trigger(SlashMissed, effect.from, data);
	}
}
...
void Room::detachSkillFromPlayer(ServerPlayer *player, const QString &skill_name){
	...
	broadcastNotification(BP::DetachSkill, detach_arg);
...
bool Room::requestPlayer(ServerPlayer *player, BP::CommandType command, const QJsonValue &arg, bool wait){
	time_t timeOut = ServerInfo.getCommandTimeout(command, BP::ServerInstance);
	return requestPlayer(player, command, arg, timeOut, wait);
}

bool Room::requestPlayer(ServerPlayer *player, BP::CommandType command, const QJsonValue &arg, time_t timeOut, bool wait){
...
bool Room::broadcastRequest(QList<ServerPlayer*> &players, BP::CommandType command)
{
   time_t timeOut = ServerInfo.getCommandTimeout(command, BP::ServerInstance);
   return broadcastRequest(players, command, timeOut);
}
...
ServerPlayer* Room::broadcastRaceRequest(QList<ServerPlayer*> &players, BP::CommandType command,
...
bool Room::notifyPlayer(ServerPlayer *player, BP::CommandType command, const QJsonValue &arg) const{
...
bool Room::broadcastNotification(BP::CommandType command, const QJsonValue &arg, ServerPlayer *except) const{
	foreach(ServerPlayer *player, m_players){
		if(player != except){
			notifyPlayer(player, command, arg);
		}
	}
	return true;
}
...
bool Room::notifyMoveFocus(ServerPlayer* player){
	return broadcastNotification(BP::MoveFocus, player->objectName());
}
...
bool Room::notifyMoveFocus(ServerPlayer* player, BP::CommandType command){
	...
	return broadcastNotification(BP::MoveFocus, arg);
}
...
bool Room::askForSkillInvoke(ServerPlayer *player, const QString &skill_name, const QVariant &data){
	...
	if(ai){
		...
		if(!requestPlayer(player, BP::SkillInvoke, skillCommand, true)){
			invoked = false;
		}else{
	...
	driver->trigger(ChoiceMade, player, decisionData);
	return invoked;
}
...
bool Room::_askForNullification(const TrickCard *trick, ServerPlayer *from, ServerPlayer *to, bool positive, _NullificationAiHelper aiHelper){
	...
	if(!validHumanPlayers.empty())
		repliedPlayer = broadcastRaceRequest(validHumanPlayers, BP::AskForNullification, timeOut, &Room::verifyNullificationResponse);
	...
	broadcastNotification(BP::Animate, BP::toJsonArray("nullification", repliedPlayer->objectName(), to->objectName()));
	...
	driver->trigger(ChoiceMade, repliedPlayer, decisionData);
...
int Room::askForCardChosen(ServerPlayer *player, ServerPlayer *who, const QString &flags, const QString &reason){
	...
	if(ai){
		...
		bool success = requestPlayer(player, BP::AskForCardChosen, BP::toJsonArray(who->objectName(), flags, reason), true);
...
const Card *Room::askForCard(ServerPlayer *player, const QString &pattern, const QString &prompt, const QVariant &data, TriggerEvent trigger_event){
	...
	driver->trigger(CardAsked, player, asked);
	if(has_provided){
		card = provided;
		provided = NULL;
		has_provided = false;
		...
		if(ai){
			...
			bool success = requestPlayer(player, BP::AskForCard, BP::toJsonArray(pattern, prompt), true);
			QJsonValue clientReply = player->getClientReply();
	...
	if(card){
		...
		if(trigger_event != NonTrigger){
			driver->trigger(trigger_event, player, card_star);
		}
...
bool Room::askForUseCard(ServerPlayer *player, const QString &pattern, const QString &prompt){
	...
	}else if(requestPlayer(player, BP::AskForUseCard, BP::toJsonArray(pattern, prompt), true)){
		QJsonValue clientReply = player->getClientReply();
...
int Room::askForAG(ServerPlayer *player, const QList<int> &card_ids, bool refusable, const QString &reason){
	...
	if(ai){
		...
		bool success = requestPlayer(player, BP::AskForAG, refusable, true);
		QJsonValue clientReply = player->getClientReply();
...
const Card *Room::askForCardShow(ServerPlayer *player, ServerPlayer *requestor, const QString &reason){
	...
	if(ai)
		...
		bool success = requestPlayer(player, BP::AskForCardShow, requestor->getGeneralName(), true);
		QJsonValue clientReply = player->getClientReply();
		if(success && clientReply.isString()){
		card = Card::Parse(clientReply.toString());
		}
...
const Card *Room::askForSingleWine(ServerPlayer *player, ServerPlayer *dying){
	...
	if(ai)
		...
		arg.append(wine_num);
		bool success = requestPlayer(player, BP::AskForSingleWine, arg, true);
		QJsonValue clientReply = player->getClientReply();
...
void Room::setPlayerFlag(ServerPlayer *player, const QString &flag){
	...
	arg.append(flag);
	broadcastNotification(BP::SetPlayerProperty, arg);
}
...
void Room::setPlayerProperty(ServerPlayer *player, const char *property_name, const QVariant &value){
	...
	if(strcmp(property_name, "hp") == 0){
		driver->trigger(HpChanged, player);
	}
}
...
void Room::setPlayerMark(ServerPlayer *player, const QString &mark, int value){
	...
	arg.append(mark);
	arg.append(value);
	broadcastNotification(BP::SetMark, arg);
}
...
void Room::setCardFlag(int card_id, const QString &flag, ServerPlayer *who){
	...
	if(who)
		...
		broadcastNotification(BP::SetCardFlag, pattern);
}
...
void Room::swapPile(){
	...
	broadcastNotification(BP::ClearPile);
	broadcastNotification(BP::SetPileNumber, QJsonValue(draw_pile->length()));
...
void Room::transfigure(ServerPlayer *player, const QString &new_general, bool full_state, bool invoke_start, const QString &old_general){
	...
	driver->addPlayerSkills(player, invoke_start);
...
void Room::setFixedDistance(Player *from, const Player *to, int distance){
	...
	broadcastNotification(BP::SetFixedDistance, set_str);
}
...
void Room::reverseFor3v3(const Card *card, ServerPlayer *player, QList<ServerPlayer *> &list){
	...
	if(player->isOnline()){
		bool success = requestPlayer(player, BP::AskForDirection, QJsonValue(), true);
		QJsonValue clientReply = player->getClientReply();
		if(success && clientReply.isString()){
...
void Room::prepareForStart(){
	if(scenario){
		...
		bool success = requestPlayer(owner, BP::AskForAssign, QJsonValue(), true);
...
void Room::reportDisconnection(){
	...
	if(player->objectName().isEmpty()){
		...
		if(m_players.length() < player_count){
			...
			broadcastNotification(BP::RemovePlayer, player->objectName());
		}
	}else{
		// fourth case
...
bool Room::makeSurrender(ServerPlayer *initiator)
{
	...
	broadcastRequest(playersAlive, BP::AskForSurrender);
...
void Room::chooseGenerals(){
	...
	broadcastRequest(to_assign, BP::AskForGeneral);
	foreach(ServerPlayer *player, to_assign){
...
void Room::swapSeat(ServerPlayer *a, ServerPlayer *b){
	...
	broadcastNotification(BP::ArrangeSeats, player_circle);
...
void Room::useCard(const CardUseStruct &card_use, bool add_history){
	...
	if(card_use.from->getPhase() == Player::Play && add_history){
		...
		broadcastNotification(BP::AddHistory, QString("pushPile"));
	}
...
void Room::loseHp(ServerPlayer *victim, int lose){
	QVariant data = lose;
	driver->trigger(HpLost, victim, data);
}
...
void Room::applyDamage(ServerPlayer *victim, const DamageStruct &damage){
	...
	broadcastNotification(BP::HpChange, change_arr);
}
...
void Room::recover(const RecoverStruct &recover, bool set_emotion){
	...
	if(recover.from != NULL){
		if(driver->trigger(BeforeRecovering, recover.from, data)){
			return;
		}
	}

	if(driver->trigger(BeforeRecovered, recover.to, data)){
		return;
	}

	if(recover.from != NULL){
		if(driver->trigger(Recovering, recover.from, data)){
			return;
		}
	}

	if(driver->trigger(Recovered, recover.to, data)){
		return;
	}
	...
	if(recover.from != NULL){
		driver->trigger(AfterRecovering, recover.from, data);
	}

	driver->trigger(AfterRecovered, recover.to, data);
}
...
bool Room::cardEffect(const CardEffectStruct &effect){
	...
	bool broken = false;
	if(effect.from)
		broken = driver->trigger(CardEffect, effect.from, data);
...
void Room::startGame(){
	...
	foreach(ServerPlayer *player, m_players){
		...
		if(mode == "02_1v1")
			broadcastNotification(BP::RevealGeneral, BP::toJsonArray(player->objectName(), player->getGeneralName()), player);
	...
	broadcastNotification(BP::StartGame);
	game_started = true;
...
void Room::drawCards(QList<ServerPlayer*> players, int n, const QString &reason)
{
	...
	foreach(ServerPlayer* player, players){
		...
		for(int i = 0; i < n; i++){
			...
			if(driver->trigger(CardDrawing, player, data))
				continue;
	...
	foreach(ServerPlayer* player, players){
		...
		driver->trigger(CardDrawnDone, player, data);
	}
}
...
void Room::throwCard(const Card *card, ServerPlayer *who){
	...
	if(who){
		...
		driver->trigger(CardDiscarded, who, data);
	}
}
...
RoomDriver *Room::getDriver() const{
	return driver;
}
	...
	foreach(const CardsMoveStruct &cards_move, cards_moves){
		...
		if(cards_move.from && (cards_move.from_place == Player::HandArea || cards_move.from_place == Player::EquipArea)){
			foreach(CardMoveStruct move, cards_move.flatten()){
				...
				driver->trigger(OneCardLost, (ServerPlayer*) cards_move.from, data);
			}

			if(cards_move.as_one_time){
				CardsMoveStar lose_star = &cards_move;
				QVariant data = QVariant::fromValue(lose_star);
				driver->trigger(CardLost, (ServerPlayer*) cards_move.from, data);
			}
		}
	}
	...
	foreach(const CardsMoveStruct &cards_move, cards_moves){
		...
		if(cards_move.to && cards_move.to != cards_move.from && (cards_move.to_place == Player::HandArea || cards_move.to_place == Player::EquipArea)){
			foreach(const CardMoveStruct &move, cards_move.flatten()){
				CardMoveStar move_star = &move;
				QVariant data = QVariant::fromValue(move_star);
				driver->trigger(OneCardGot, (ServerPlayer*) cards_move.to, data);
			}

			if(cards_move.as_one_time){
				CardsMoveStar move_star = &cards_move;
				QVariant data = QVariant::fromValue(move_star);
				driver->trigger(CardGot, (ServerPlayer*) cards_move.to, data);
			}
		}
	}
}
...
bool Room::notifyMoveCards(bool is_lost_phase, QList<CardsMoveStruct> cards_moves, bool force_visible){
	...
	foreach(ServerPlayer* player, m_players){
		...
		if(is_lost_phase)
			notifyPlayer(player, BP::LoseCard, arg);
		else
			notifyPlayer(player, BP::GetCard, arg);
	}
	return true;
}
...
void Room::playSkillEffect(const QString &skill_name, int index) const{
	...
	arg.append(index);
	broadcastNotification(BP::PlaySkillEffect, arg);
}
...
void Room::broadcastSkillInvoked(ServerPlayer *player, const QString &skill_name) const{
	...
	broadcastNotification(BP::SkillInvoke, msg);
}
...
void Room::acquireSkill(ServerPlayer *player, const Skill *skill, bool open){
	...
	if(skill->inherits("TriggerSkill")){
		...
		driver->addTriggerSkill(trigger_skill);
	}
	...
	if(open){
		...
		broadcastNotification(BP::AcquireSkill, acquire_arr);
	}
...
void Room::setEmotion(ServerPlayer *target, const QString &emotion){
	...
	broadcastNotification(BP::SetEmotion, emo);
}
...
void Room::activate(ServerPlayer *player, CardUseStruct &card_use){
	...
	if(ai){
		...
		bool success = requestPlayer(player, BP::Activate, player->objectName(), true);
		QJsonValue clientReply = player->getClientReply();
	...
	driver->trigger(ChoiceMade, player, data);
}
...
Card::Suit Room::askForSuit(ServerPlayer *player, const QString& reason){
	...
	bool success = requestPlayer(player, BP::AskForSuit, QJsonValue(), true);
...
QString Room::askForKingdom(ServerPlayer *player){
	...
	bool success = requestPlayer(player, BP::AskForKingdom, QJsonValue(), true);
...
bool Room::askForDiscard(ServerPlayer *player, const QString &reason, int discard_num, int min_num, bool optional, bool include_equip){
	...
	if(ai){
		...
		bool success = requestPlayer(player, BP::AskForDiscard, ask_str, true);
...
const Card *Room::askForExchange(ServerPlayer *player, const QString &reason, int discard_num){
	...
	if(ai){
		...
		bool success = requestPlayer(player, BP::AskForExchange, discard_num, true);
		//@todo: also check if the player does have that card!!!
		QJsonArray clientReply = player->getClientReply().toArray();
...
void Room::askForGuanxing(ServerPlayer *zhuge, const QList<int> &cards, bool up_only){
	...
	if(ai){
		...
		bool success = requestPlayer(zhuge, BP::AskForGuanxing, guanxingArgs, true);
...
void Room::doGongxin(ServerPlayer *shenlvmeng, ServerPlayer *target){
	...
	bool success = requestPlayer(shenlvmeng, BP::AskForGongxin, gongxinArgs, true);
...
const Card *Room::askForPindian(ServerPlayer *player, ServerPlayer *from, ServerPlayer *to, const QString &reason)
{
	...
	bool success = requestPlayer(player, BP::AskForPindian, BP::toJsonArray(from->objectName(), to->objectName()), true);
...
ServerPlayer *Room::askForPlayerChosen(ServerPlayer *player, const QList<ServerPlayer *> &targets, const QString &skillName){
	...
	if(ai)
		...
		bool success = requestPlayer(player, BP::AskForPlayerChosen, req, true);
...
QString Room::askForGeneral(ServerPlayer *player, const QStringList &generals, QString default_choice){
	...
	if(player->isOnline()){
		...
		bool success = requestPlayer(player, BP::AskForGeneral, options, true);
...
void Room::fillAG(const QList<int> &card_ids, ServerPlayer *who){
	...
	if(who)
		...
		broadcastNotification(BP::FillAG, cards);
	}
}
...
void Room::takeAG(ServerPlayer *player, int card_id){
	if(player){
		...
		take.append(card_id);
		broadcastNotification(BP::TakeAG, take);
		...
		CardMoveStar move_star = &move;
		QVariant data = QVariant::fromValue(move_star);
		driver->trigger(OneCardGot, player, data);
	}else{
		discard_pile->prepend(card_id);
...
void Room::sendLog(const LogMessage &log){
	...
	broadcastNotification(BP::Log, log.toString());
}
...
void Room::showCard(ServerPlayer *player, int card_id, ServerPlayer *only_viewer){
	...
	if(only_viewer)
		notifyPlayer(player, BP::AskForCardShow, show_str);
	else{
		if(card_id > 0)
			setCardFlag(card_id, "visible");
		broadcastNotification(BP::AskForCardShow, show_str);
	}
}
...
void Room::showAllCards(ServerPlayer *player, ServerPlayer *to){
	...
	if(isUnicast)
		notifyPlayer(to, BP::ShowAllCards, gongxinArgs);
	else{
		foreach(int card_id, player->handCards())
			setCardFlag(card_id, "visible");
		broadcastNotification(BP::ShowAllCards, gongxinArgs);
	}
}
...
bool Room::askForYiji(ServerPlayer *guojia, QList<int> &cards){
	...
	if(ai){
		...
		bool success = requestPlayer(guojia, BP::AskForYiji, BP::toJsonArray(cards), true);
...
void Room::arrangeCommand(ServerPlayer *player, const QJsonValue &arg){
	...
	if(mode == "06_3v3")
		driver_3v3->arrange(player, arranged);
	else if(mode == "02_1v1")
		driver_1v1->arrange(player, arranged);
}

void Room::takeGeneralCommand(ServerPlayer *player, const QJsonValue &arg){
	if(mode == "06_3v3")
		driver_3v3->takeGeneral(player, arg.toString());
	else if(mode == "02_1v1")
		driver_1v1->takeGeneral(player, arg.toString());
}

QString Room::askForOrder(ServerPlayer *player){
	notifyMoveFocus(player, BP::AskForOrder);
	bool success = requestPlayer(player, BP::AskForOrder, (int) BP::ChooseOrderTurn, true);
...
QString Room::askForRole(ServerPlayer *player, const QStringList &roles, const QString &scheme){
	...
	bool success = requestPlayer(player, BP::AskForRole3v3, arg, true);
	QJsonValue clientReply = player->getClientReply();
